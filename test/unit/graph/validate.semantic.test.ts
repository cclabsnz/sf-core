...
const doc = (nodes: unknown[], edges: unknown[] = []) => ({
  schemaVersion: SUPPORTED_SCHEMA_VERSION, capturedAt: '2026-09-07T00:00:00Z',
...
describe('validateGraph — semantic', () => {
  it('rejects a stored level that disagrees with the kind table', () => {
    expect(codes(doc([node({ level: 0 })]))).toContain(RULES.LEVEL_KIND_MISMATCH);
  });

  it('rejects a stored layer that disagrees with the kind table', () => {
    expect(codes(doc([node({ layer: 'access' })]))).toContain(RULES.LAYER_KIND_MISMATCH);
  });

  it('rejects an unknown kind', () => {
    expect(codes(doc([node({ kind: 'widget' })]))).toContain(RULES.UNKNOWN_KIND);
  });

  it('rejects an id that is not namespaced by kind', () => {
    expect(codes(doc([node({ id: 'Account' })]))).toContain(RULES.ID_NOT_NAMESPACED);
  });

  it('rejects duplicate ids', () => {
    expect(codes(doc([node({}), node({})]))).toContain(RULES.ID_DUPLICATE);
  });
  ...
  it('rejects a level 0 node that carries a parent', () => {
    ...
    expect(codes(doc([org, node({})]))).toContain(RULES.ROOT_HAS_PARENT);
  });

  it('rejects a parent that does not resolve', () => {
    expect(codes(doc([node({ parent: 'domain.missing' })]))).toContain(RULES.PARENT_UNRESOLVED);
  });

  it('rejects a parent at the same or a higher level', () => {
    const a = node({ id: 'obj.A', parent: 'obj.B' });
    const b = node({ id: 'obj.B' });
    expect(codes(doc([a, b]))).toContain(RULES.LEVEL_PARENT_ORDER);
  });
  ...
  it('rejects a parent cycle', () => {
    ...
    expect(codes(doc([a, b]))).toContain(RULES.PARENT_CYCLE);
  });
  ...
  it('requires a derived edge to name its rule', () => {
    ...
    expect(codes(doc([node({})], [e]))).toContain(RULES.DERIVED_MISSING_RULE);
  });
  ...
  it('rejects an edge endpoint that does not resolve', () => {
    ...
    expect(codes(doc([node({})], [e]))).toContain(RULES.EDGE_ENDPOINT_UNRESOLVED);
  });
});
